...
<aside class="fixed inset-y-0 left-0 z-40 h-screen w-64 bg-white border-r shadow-sm transition-transform duration-300
    ...
    <nav class="py-3 overflow-y-auto h-[calc(100vh-5rem)]">
        ...
        <!-- Conectar -->
        <x-sidebar-item href="{{ ($role === 'company') ? route('connect.jobs') : route('connect.index') }}" :active="request()->routeIs('connect.index') || request()->routeIs('connect.jobs')" icon="fa-solid fa-share-nodes">Conectar</x-sidebar-item>
